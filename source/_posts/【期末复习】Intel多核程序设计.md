...
SISD计算机通常由一个处理单元和一个存储器组成，如下图所示。它通过执行单一的指令流，对单一的数据流进行操作，指令按顺序读取，数据在每一时刻也只能读取一个。
    ...
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/df3bb908065afe3417cfbe67aa05ce46.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/356f2fcf8850cdd2ee8110e4b3e55242.png)
#### 多指令流单数据流 (MISD ，Multiple Instruction stream Single Data stream) 
  MISD计算机具有多个处理单元，这些处理单元组成一个线性阵列，分别执行不同的指令流，而同一个数据流则顺序通过这个阵列中的各个处理单元，如下图所示：
      ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3b12565454eb57ecbe88465262daba75.png)
...
#### 多指令流多数据流 (MIMD ，Multiple Instruction stream Multiple Data stream) 
 ...
 MIMD系统是计算机能够实现指令、数据作业、任务等各级全面并行计算的多处理系统。典型的MIMD系统由多个处理单元、多个存储模块和一个互联网络组成，如下图所示：
  ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2b7fec5c5a6fc48029e3d56a6a049d6b.png)
...
1.  顺序控制方式
        ...
        ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/be624906472b3c90d912ef38a4d97eca.png)
2.  重叠控制方式
     在第K条指令操作完成之前，就可对第K+1条指令进行操作，这就是重叠控制方式。
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5bf6daccf6d1b903a19a6091483050ae.png)
3. 流水控制方式
     若把重叠中的分析再细分（当然，所细分成的部分必须有相应独立的功能段），也就构成了流水控制方式。
     我们把一条指令的解释分为如图所示的5个过程，
     ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/913250b3505b41864af8fa68563d3ca6.png)
那么采用流水过程的时空关系就如下图所示。
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c17b299cfe4d03d255222c65d649e96e.png)
4. 相关及其处理
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/6a99685dcb1ed9c1e9783bf20a6da2aa.png)
...
### 15.线程具有哪四种基本状态？
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e875f0f4465c450384a1134f4daf8480.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a171441d26b22ad962c22e3eaf1d80da.png)
...
S3: B=(D+2)
S4: A=COS(C)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/011e28d77d12110987a5ef3ab89d4660.png)
...
(1)画出数据关系表达式和数据相关图；
![(1)](https://i-blog.csdnimg.cn/blog_migrate/acbe1095218e797151f3cdd76bddf4b4.png)
(2)写出用fork、join语句表示其并行任务的派生和汇合关系的程序；
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/ba3e225833db7210aef35bba8b2dc3a5.png)
